...
import { ArrowLeft, CheckCircle2, Circle, ListMusic } from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[var(--color-background)]">
      <header className="sticky top-0 z-10 flex items-center gap-3 p-4 bg-[var(--color-background)]/90 backdrop-blur border-b border-[var(--color-border)]">
        ...
        {/* Atajo al repaso libre con esta vuelta preseleccionada */}
        <Link
          href={`/repaso-libre/nuevo?vuelta=${vId}`}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border border-[var(--color-border)] hover:border-[var(--color-primary)] transition-all"
        >
